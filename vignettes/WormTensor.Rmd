...
# worm_download
A worm_download is a function to retrieve data from figshare for a total of 28 animals (24 normal and 4 noisy).
...
# as_worm_tensor
An as_worm_tensor is a function to generate a WormTensor object from distance matrices.
A WormTensor object S4 class used by as_worm_tensor.R, worm_membership.R, worm_clustering.R, worm_evaluate.R, worm_visualize.R.
```{r as_worm_tensor, echo=TRUE}
object <- as_worm_tensor(object$Ds)
```

# worm_membership
A worm_membership is a function to generate a membership tensor from a WormTensor object with distance matrices.
...
# worm_clustering
A worm_clustering is a function to generate a clustering result from a WormTensor object with a membership tensor.
```{r worm_clustering, echo=TRUE}
object <- worm_clustering(object)
```

# worm_evaluate
A worm_evaluate is a function to generate an evaluation result from a WormTensor object with a worm_clustering result. 
```{r worm_evaluate, echo=TRUE}
object <- worm_evaluate(object)
```

# worm_visualize
A worm_visualize is a function to visualize a worm_evaluate result.
...
# worm_distance
A worm_distance helps you analyze your time-series data matrices with WormTensor.
A worm_distance is a function to convert time-series data matrices into distance matrices.
